...
export function getValueBasedOnAccessKey(
  accessKey: "subject" | "predicate" | "object",
  triple?: RDFResource,
): string {
  if (!triple) return "";
  ...
  if (accessKey === "object") {
    const key = Object.keys(triple).filter(
      (k) => !KNOWN_KEYS.includes(k),
    )[0] as keyof RDFResource;
...
export function getLabelBasedOnAccessKey(
  id: string,
  triples: RDFResource[],
): string {
  if (!id) return "";
  ...
  const label = (
    triple?.["http://www.w3.org/2000/01/rdf-schema#label"] as RDFProperty[]
  )?.[0]?.["@value"] as string;
  if (!label) {
